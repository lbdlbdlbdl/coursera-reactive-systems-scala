...
object Async extends AsyncInterface {
  ...
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] =
    eventuallyX.recover { case _ => -1 } //or .case(_ => -1)
  ...
  def sequenceComputations[A, B](
    ...
    for {
      v1 <- makeAsyncComputation1()
      v2 <- makeAsyncComputation2()
    } yield (v1, v2)
/* or
    makeAsyncComputation1().flatMap { v1 =>
      makeAsyncComputation2().map(v2 => (v1, v2))
    }
*/
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi = {
    val p = Promise[Int]()
    callbackBasedApi.computeIntAsync(continuationTry => p.tryComplete(continuationTry));
    () => p.future
  }
